...
flowchart LR
    subgraph IN[" "]
        direction TB
        U["📂 <b>Upload</b><br/>CSV / Excel / PDF"]
    end

    subgraph CORE[" "]
        direction TB
        P["📄 <b>Document Processing</b><br/>PDFPlumber parsing"]
        CL["🏷️ <b>Classification Agent</b><br/>Incoming/Outgoing<br/>+ 11 categories"]
        DB["🗄️ <b>Structured Data Store</b><br/>PostgreSQL"]
    end

    subgraph AGENTS[" "]
        direction TB
        FC["🔮 <b>Forecasting Agent</b><br/>Prophet / ARIMA"]
        IN2["💬 <b>Insight Agent</b><br/>OpenAI API"]
        SIM["🎛️ <b>Scenario Agent</b><br/>What-if simulation"]
    end

    subgraph OUT[" "]
        direction TB
        DASH["📊 <b>Dashboard</b><br/>KPIs & charts"]
        ALR["🔔 <b>Alerts &<br/>Recommendations</b>"]
    end
    ...
    classDef input fill:#1E3A8A,stroke:#60A5FA,stroke-width:2px,color:#EFF6FF,rx:10,ry:10
    classDef core fill:#065F46,stroke:#34D399,stroke-width:2px,color:#ECFDF5,rx:10,ry:10
    classDef agent fill:#9D174D,stroke:#F472B6,stroke-width:2px,color:#FDF2F8,rx:10,ry:10
    classDef output fill:#92400E,stroke:#FBBF24,stroke-width:2px,color:#FFFBEB,rx:10,ry:10
    class U input
    class P,CL,DB core
    class FC,IN2,SIM agent
    class DASH,ALR output
...
#### 🔮 Forecasting Engine
Uses Prophet / ARIMA time-series models to project account balances 30 or 90 days into the future, with confidence bands that widen further out and narrow as new transactions arrive.
...
flowchart LR
    subgraph L[" "]
        direction TB
        A["🧾 <b>Messy real-world</b><br/><b>statements</b>"]
        B["🔁 <b>Recurring expense</b><br/><b>detection</b>"]
        C["📉 <b>Forecast</b><br/><b>uncertainty</b>"]
        D["🌐 <b>Multi-currency</b><br/><b>consistency</b>"]
    end

    A --> A1["Bank PDFs vary wildly in layout;<br/>handled inconsistent columns,<br/>merged fields & OCR-prone formats"]
    B --> B1["Careful rule design to separate<br/>genuine recurring charges from<br/>coincidentally similar amounts"]
    C --> C1["Short early transaction histories<br/>made forecasts noisy; confidence<br/>intervals widened & communicated clearly"]
    D --> D1["Careful state management to keep<br/>KPIs, charts & recommendations<br/>in sync across INR, USD, EUR"]

    classDef challenge fill:#1E1B4B,stroke:#A5B4FC,stroke-width:2px,color:#EEF2FF,rx:10,ry:10
    classDef resolution fill:#082F49,stroke:#38BDF8,stroke-width:2px,color:#F0F9FF,rx:10,ry:10
