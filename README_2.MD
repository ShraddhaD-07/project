...
|:---|:---|:---|
| 📏 **Mean Absolute Error (MAE)** | Average magnitude of forecast errors, in the same units as the balance itself | ~4.2% |
| 📐 **Root Mean Squared Error (RMSE)** | Penalizes larger forecast misses more heavily than small ones | ~6.8% |
| 🧮 **Mean Squared Error (MSE)** | Squared error term used internally for model tuning and comparison | ~1.3% |
| ⚖️ **Mean Absolute Scaled Error (MASE)** | Compares forecast error against a naive baseline, making accuracy comparable across users with different spending scales | ~0.85 |
